...
from ..data import get_films, get_film, save_film, delete_film, update_film, find_film_by_title, find_films_by_partial_title
...
async def check_film_title(message: Message, state: FSMContext) -> None:
    title = message.text
    film = find_film_by_title(title)

    if film:
        await state.update_data(title=title)
        await state.set_state(FilmCreateUpdateForm.desc)
        await message.answer(
...
@film_router.message(FilmCreateUpdateForm.desc)
...
async def process_url(message: Message, state: FSMContext) -> None:
    ...
    if validators.url(url):
        ...
        await state.set_state(FilmCreateUpdateForm.photo)
        await edit_or_answer(
            message,
...
@film_router.message(FilmCreateUpdateForm.photo)
...
async def process_rating(message: Message, state: FSMContext) -> None:
    ...
    film = find_film_by_title(data.get('title'))
    if film:
        text = f"Name: {hbold(film.get('title'))}\nDescription: {hbold(film.get('desc'))}\nRating: {hbold(film.get('rating'))}"
        photo_id = film.get('photo')
        url = film.get('url')
